...
error = [];
dim =25; %%check recognition rate every dim dimensions (change it appropriatly for PCA, LDA etc
for jj = 1:20
    jj
    ...
    U_reduc = lppCW(fea_Train,ceil(length(gnd_Train)/max(gnd_Train)),1,size(fea_Train,1)-1);
